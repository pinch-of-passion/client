...
function ShowSpoonacularRecipe({ recipeId }) {
    ...
    useEffect(() => {
        async function fetchData() {
            const ans = await axios.get(`https://api.spoonacular.com/recipes/${recipeId}/information?apiKey=${process.env.REACT_APP_API_KEY3}`)
            if (ans.data) {
                setRecipe(ans.data);
            }
    ...
    return (
        <>
            ...
            <Paper elevation={0} sx={{ my: { xs: 3, md: 6 }, p: { xs: 2, md: 3 }, }}>
                ...
                <Typography variant='h11'>{recipe?.summary}</Typography>
...
//             {/* <Paper elevation={0} sx={{ my: { xs: 3, md: 6 }, p: { xs: 2, md: 3 },  }}> */}
//                 {/* <Typography variant='h3' >{recipe.title}</Typography>
//                 <Typography variant='h11' sx={{ margin: 5 }}>{(summary)}</Typography><br/> */}
//                 {/* <img style={{ }} src={recipe?.image}></img> */}
//                 {/* <Details recipe={recipe}/> */}
//                 {/* {recipe.extendedIngredients && <DisplayIngredients ingredients={recipe.extendedIngredients} id={recipe.id}></DisplayIngredients>} */}
//                 {/* {recipe.analyzedInstructions.length>0 &&<DisplayInstruction steps={recipe.analyzedInstructions}></DisplayInstruction>} */}
//                 {recipe.diets.length>0 && <DisplayTags tags={recipe.diets}></DisplayTags>}
//                 {recipe.dishTypes>0 && <DisplayCategories categories={recipe.dishTypes}></DisplayCategories>}
//                 {/* <DisplayComment commentList={recipe.comments}></DisplayComment> */}
//             {/* </Paper> */}
